...
private const val POSTGRES_DOUBLE_TYPE = "float8"

internal class PostgresDoubleListTypeDescriptor : SqlTypeDescriptor {
    companion object {
        val INSTANCE =
            PostgresDoubleListTypeDescriptor()
    }

    override fun getSqlType(): Int = DOUBLE_LIST
    ...
    override fun <X> getBinder(javaTypeDescriptor: JavaTypeDescriptor<X>): ValueBinder<X> {
        return object : BasicBinder<X>(javaTypeDescriptor, this) {
            ...
            override fun doBind(st: PreparedStatement, value: X, index: Int, options: WrapperOptions) {
                ...
                val array = st.connection.createArrayOf(POSTGRES_DOUBLE_TYPE, elements.toTypedArray())
