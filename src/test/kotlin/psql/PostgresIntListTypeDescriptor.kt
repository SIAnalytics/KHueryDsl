...
private const val POSTGRES_INT_TYPE = "integer"

internal class PostgresIntListTypeDescriptor : SqlTypeDescriptor {
    companion object {
        val INSTANCE =
            PostgresIntListTypeDescriptor()
    }

    override fun getSqlType(): Int = INT_LIST
    ...
    override fun <X> getBinder(javaTypeDescriptor: JavaTypeDescriptor<X>): ValueBinder<X> {
        return object : BasicBinder<X>(javaTypeDescriptor, this) {
            ...
            override fun doBind(st: PreparedStatement, value: X, index: Int, options: WrapperOptions) {
                ...
                val array = st.connection.createArrayOf(POSTGRES_INT_TYPE, elements.toTypedArray())
